...
class Analyzer:
    ...
    def filter_changed_incidents(self, events: List[IncidentEvent]) -> List[IncidentEvent]:
        ...
        for event in events:
            ...
            is_updated = bool(event.last_contact and event.last_contact > stored_ts)

            if is_new or is_updated:
                changed_events.append(event)
                if event.last_contact:
                    self._incident_times[event.incident_uuid] = event.last_contact
                    state_updated = True

            if event.last_contact and event.last_contact > new_max_time:
                new_max_time = event.last_contact
                state_updated = True
